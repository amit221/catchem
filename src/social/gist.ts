...
const IMG_BASE = "https://raw.githubusercontent.com/amit221/catchem-assets/main/images";
const COLS = 4;
...
export function generateGistMarkdown(state: GameState, username: string): string {
  ...
  const caughtCreatures = Object.entries(state.creatures)
    ...
    .filter((c) => c.def)
    .sort((a, b) => rarityOrder.indexOf(a.def!.rarity) - rarityOrder.indexOf(b.def!.rarity));
  ...
  if (rarestCatches.length > 0) {
    ...
    for (const c of rarestCatches) {
      ...
      md += `<td align="center"><img src="${IMG_BASE}/${c.id}.png" width="80" alt="${c.def!.name}"><br>`;
      md += `<b>${c.def!.name}</b><br>${icon} ${c.def!.rarity} · Lv.${c.level}</td>\n`;
    }
    md += `</tr></table>\n\n`;
  }
  ...
  md += `## 📦 Collection\n\n`;
  md += `<table>\n`;
  for (let i = 0; i < caughtCreatures.length; i++) {
    if (i % COLS === 0) md += `<tr>\n`;
    const c = caughtCreatures[i];
    const icon = RARITY_ICONS[c.def!.rarity] ?? "⚪";
    const theme = THEME_LABELS[c.def!.theme] ?? c.def!.theme;
    md += `<td align="center" width="25%">`;
    md += `<img src="${IMG_BASE}/${c.id}.png" width="80" alt="${c.def!.name}"><br>`;
    md += `<b>${c.def!.name}</b><br>`;
    md += `${icon} ${c.def!.rarity} · Lv.${c.level}<br>`;
    md += `<sub>${theme} · x${c.catchCount}</sub>`;
    md += `</td>\n`;
    if (i % COLS === COLS - 1 || i === caughtCreatures.length - 1) md += `</tr>\n`;
  }
  md += `</table>\n`;
