...
describe("getBorderColor", () => {
  it("returns cyan-family color for selected cards", () => {
    ...
    expect(["cyan", "cyanBright", "white"]).toContain(color);
  });
  ...
  it("returns animated color for unselected legendary cards", () => {
    ...
    expect(colors.every(c => ["yellow", "yellowBright", "white"].includes(c))).toBe(true);
  ...
  it("returns animated color for unselected mythic cards", () => {
    ...
    expect(colors.every(c => ["red", "magenta", "redBright"].includes(c))).toBe(true);
    expect(new Set(colors).size).toBeGreaterThan(1);
  });
});
